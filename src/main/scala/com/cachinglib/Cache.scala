...
class Cache[K, V] private (
  ...
  /** Retrieves the value associated with a key.
    ...
    *   The key to look up.
    * @return
    *   An `Option` containing the value if found, or `None` if not.
    */
  ...
  /** Adds or updates a key-value pair in the cache. If the cache is at
    * capacity, it evicts an item based on the configured policy.
    *
    * @param key
  ...
  def put(key: K, value: V): Unit =
    ...
    try
      ...
      if keyExisted then policy.trackAccess(key)
      else policy.trackInsertion(key)
    finally lock.unlock()
...
object Cache:
  ...
  class Builder[K, V]:
    ...
    /** Sets the maximum number of items the cache can hold. This is a required
      * parameter.
      */
    ...
    /** Sets the eviction policy for the cache. This is a required parameter.
      */
    def withPolicy(p: EvictionPolicy[K]): Builder[K, V] =
      this.policy = Some(p)
      this

    /** Sets the storage implementation for the cache. Optional; defaults to
      * InMemoryStorage if not set.
      */
    ...
    /** Builds and returns a new `Cache` instance.
      ...
      *   if capacity or policy have not been set.
      */
    def build(): Cache[K, V] =
      val finalCapacity = capacity.getOrElse(
        throw new IllegalStateException("Capacity must be set.")
      )
      val finalPolicy = policy.getOrElse(
